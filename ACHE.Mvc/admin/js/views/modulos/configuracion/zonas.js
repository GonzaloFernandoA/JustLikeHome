function configControls() {
    ...
    $("#grid").kendoGrid({
        dataSource: {
            ...
            transport: {
                read: {
                    url: "zonas.aspx/Get", //specify the URL which data should return the records. This is the Read method of the Products.asmx service.
    ...
    $("#grid").delegate(".editColumn", "click", function (e) {
        ...
        window.location.href = "zonase.aspx?Mode=E&Id=" + dataItem.IDZona;
    });
    ...
    $("#grid").delegate(".deleteColumn", "click", function (e) {
        ...
        if (confirm("¿Esta seguro que desea eliminar el item seleccionado?")) {
            $.ajax({
                type: "POST",
                url: "zonas.aspx/Delete",
                data: "{ id: " + dataItem.IDZona + "}",
...
function nuevo() {
    window.location.href = "zonase.aspx?Mode=A";
}
